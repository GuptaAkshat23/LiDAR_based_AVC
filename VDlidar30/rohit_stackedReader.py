...
def save_combined_pcd(points_list, output_folder):
    ...
    filename = os.path.join(output_folder, f"tg30_full_360_3d_{timestamp}.pcd")

    num_points = len(points_list)
    print(f"\n[SAVING] Writing {num_points} points to {filename}...")

    with open(filename, 'w') as f:
        f.write("# .PCD v.7 - TG30 Full 360 Scan\nVERSION .7\nFIELDS x y z\nSIZE 4 4 4\nTYPE F F F\nCOUNT 1 1 1\n")
...
def process_packet(packet):
    """Parses TG30 binary packet and returns ALL 360-degree points."""
    try:
        lsn = packet[3]
        fsa = struct.unpack('<H', packet[4:6])[0]
        lsa = struct.unpack('<H', packet[6:8])[0]

        # Angle processing per YDLidar TG30 manual
        ...
        for i in range(lsn):
            ...
            if distance > 0:
                ...
                # Keep raw_angle within 0-360 range
                raw_angle = raw_angle % 360

                angle_rad = np.radians(raw_angle)

                # Sensor Coordinates (2D slice)
                # Converting polar (dist, angle) to Cartesian (y, z)
...
def run_recorder():
    ...
    try:
        ...
        ser.setDTR(True)
        ser.write(b'\xA5\x60')  # Start command
        time.sleep(0.5)
        ser.reset_input_buffer()

        print(f"🚀 TG30 3D Stacked Recorder Started (FULL 360°)...")
            ...
            if ser.in_waiting:
                ...
                while len(buffer) > 10:
                    if buffer[0] == 0xAA and buffer[1] == 0x55:
                        ...
                        for pt in points:
                            # Detect Rotation Wrap-around (e.g., angle goes from 359 back to 1)
                            # This signals one full rotation has completed
                            if pt['raw_angle'] < last_raw_angle and (last_raw_angle - pt['raw_angle'] > 180):
                                scan_counter += 1
                                if scan_counter % 10 == 0:
                                    print(f"Rotation: {scan_counter} | Total Points: {len(all_points_buffer)}")

                            # Map to 3D Space
                            # X = Progress (Travel Axis)
